...
// (https://github.com/PixeroJan/obsidian-toword). Source file in upstream repo:
// converter-mobile.ts (the repo has no converter.ts). Adapted here as src/converter.ts;
// the only functional change is external hyperlink relationships (see getExternalLinkId).
...
export class MarkdownToDocxConverter {
	...
	private externalLinks: Map<string, string> = new Map(); // url -> rId (hyperlinks externos, p.ej. obsidian://)
	...
	// ponytail: rIds desde 1000 para no chocar con styles(1)/numbering(2)/imágenes(rId11+);
	// techo ~989 enlaces distintos por documento.
	private getExternalLinkId(url: string): string {
		let id = this.externalLinks.get(url);
		if (!id) {
			id = `rId${1000 + this.externalLinks.size}`;
			this.externalLinks.set(url, id);
		}
		return id;
	}
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/>
	...
	private getDocumentRelsXml(): string {
		...
		let linkRels = '';
		this.externalLinks.forEach((id, url) => {
			linkRels += `  <Relationship Id="${id}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${this.escapeXml(url)}" TargetMode="External"/>\n`;
		});
...
${imageRels}${linkRels}</Relationships>`;
	}
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" 
  ...
  <w:r>
	...
	private convertMarkersToWordXml(text: string): string {
		...
		// Process links
		result = result.replace(/\|\|\|LINK\|\|\|([\s\S]*?)\|\|\|DATA:([\s\S]*?)\|\|\|\/LINK\|\|\|/g, (_match: string, content: string, url: string) =>
			`<w:hyperlink r:id="${this.getExternalLinkId(url)}"><w:r><w:rPr><w:color w:val="0000FF"/><w:u w:val="single"/></w:rPr><w:t xml:space="preserve">${this.escapeXml(content)}</w:t></w:r></w:hyperlink>`);
